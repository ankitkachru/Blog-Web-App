...
class JBlogJSON(webapp2.RequestHandler):
    def get(self):
        ...
        json_list = []
        blog_entries = db.GqlQuery("SELECT * FROM Page ORDER BY created DESC limit 10")
        for entry in blog_entries:
            json_list.append({'content':entry.content,'subject':entry.name,'created':entry.created("%A %d %B %Y")})
        self.response.out.write(json.dumps(json_list))

class JSinglePostJSON(webapp2.RequestHandler):
    def get(self, Page_id):
        self.response.headers['Content-Type'] = "application/json"
        entry = Page.get_by_id(int(Page_id))
        json_post = {}
        if entry:
            json_post['content']=entry.content
            json_post['subject']=entry.subject
            json_post['created']=entry.created.strftime("%A %d %B %Y")
